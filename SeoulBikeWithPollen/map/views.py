...
def index(request):
    ...
    for station in stations:
        ...
        if station.parkingBikeTotCnt == 0:
            folium.Marker(coordinate, popup = popup, icon = folium.Icon(color = 'red', icon='', prefix='fa')).add_to(mCluster)
            continue
        folium.Marker(coordinate, popup = popup, icon = folium.Icon(color = 'green', icon='', prefix='fa')).add_to(mCluster)
